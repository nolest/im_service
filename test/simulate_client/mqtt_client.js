...
MqttClient.prototype.login = function(options) {
    ...
    function onMessageArrived(message) {
        ...
        self.deleteMessage(self.client, message.destinationName, msg_data)
    }
    ...
    self.mqtt_id = options.user_type + '/' + identify + '/app/' + Date.now();
...
MqttClient.prototype.deleteMessage = function(client, chat_id, msg_data) {
    if (msg_data['property'] && msg_data['property']['feedback'] && msg_data['property']['feedback']['sender_seq']) {
        if (chat_id === msg_data.from + '/' + msg_data.sender) {
            msg_data.peer_seq = msg_data['property']['feedback']['sender_seq'];
        }
    }

    this.client.subscribe('$READPOINT/' + chat_id + ':' + msg_data.peer_seq);
}
        ...
        for (var i = 0; i < suback_obj.length; ++i) {
            if (suback_obj[i].messages) {
                ...
                for (var j = 0; j < all_msg.length; ++j) {
                    ...
                    // 收到消息后需要主动删除消息，否则下次登录时仍然会收到。
                    self.deleteMessage(self.client, topic, msg_data);
                }
            }
        }
